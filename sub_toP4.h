...
*/
class SubToP4 : public Inspector {
  ...
  public:
    ...
    /* FIXME  -- simplify this by getting rid of the 'builder' object and just
     * emitting directly to the ostream.  The SourceCodeBuilder object does not
     * appear to add any useful functionality the ostream does not already
     * provide; it just serves to obfuscate the code */
    ...
    SubToP4(Util::SourceCodeBuilder &builder, bool showIR,
            cstring mainFile = nullptr)
        : expressionPrecedence(DBPrint::Prec_Low), isDeclaration(true),
          showIR(showIR), withinArgument(false), builder(builder),
          outStream(nullptr), mainFile(mainFile) {
        visitDagOnce = false;
        setName("SubToP4");
    }
    SubToP4(std::ostream *outStream, bool showIR, cstring mainFile = nullptr)
        : expressionPrecedence(DBPrint::Prec_Low), isDeclaration(true),
          showIR(showIR), withinArgument(false),
          builder(*new Util::SourceCodeBuilder()), outStream(outStream),
          mainFile(mainFile) {
        visitDagOnce = false;
        setName("SubToP4");
    }
